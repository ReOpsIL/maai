...
class ReviewerAgent(BaseAgent):
    ...
    adherence to specifications (impl_*.md), and best practices.
    """
    ...
    def run(self, generated_files: list[str] | None = None, impl_content: str | None = None) -> str | None:
        ...
        Args:
            ...
            impl_content: The content of the implementation plan (impl_*.md). If None,
                          it attempts to read 'docs/impl_*.md'.
        ...
        if impl_content is None:
            ...
            if not impl_content:
                ...
                raise FileNotFoundError(f"Implementation plan is required for review.")
    ...
    def _perform_ai_review(self, impl_content: str, source_code: dict[str, str]) -> str | None:
        ...
        try:
            ...
            success_keywords = [
                "code looks good", "no major issues found", "review passed", "looks correct",
                "adheres to the plan", "approved", "well done", "meets requirements",
                "no further comments", "lgtm", "looks good to me", "ready to merge",
                "clean implementation", "no changes needed", "nothing to add",
                "well structured", "solid work", "passes all checks", "implementation is sound",
                "logic appears correct", "functionality is intact", "meets the objectives",
                "design is consistent", "implementation aligns with the specification",
                "well-executed", "follows best practices", "minimal feedback", "acceptable as-is"
            ]

            # Keywords indicating feedback is present (includes phrasing from LLMs like Gemini, ChatGPT, Claude)
            feedback_keywords = [
                "issues found", "needs changes", "recommendations:", "feedback:",
                "consider changing", "does not adhere", "needs improvement",
                "refactor", "bug:", "error:", "concern:", "inconsistent with standards",
                "code smell", "violates", "unclear logic", "not efficient", "suboptimal",
                "missing tests", "naming issue", "too complex", "ambiguous",
                "potential issue", "consider simplifying", "edge cases not handled",
                "could be improved", "redundant code", "performance impact",
                "clarify intent", "improve readability", "not scalable", "breaks convention",
                "introduces risk", "deviation from expected behavior", "recommend revisiting"
            ]
