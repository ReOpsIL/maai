...
class ReviewerAgent(BaseAgent):
    ...
    def run(self, generated_files: list[str], impl_content: str) -> tuple[bool, str | None]:
        """
        Executes the Reviewer agent's task using an LLM.

        Args:
            generated_files: A list of absolute paths to the Python files
                             generated by the Coder Agent.
            impl_content: The content of the implementation plan (impl.md).

        Returns:
            A tuple containing:
            - bool: True if the review passes (no significant issues found by LLM), False otherwise.
            - str | None: Formatted feedback string if issues are found, None otherwise.
        """
        self.logger.info(f"Running AI Reviewer Agent for project: {self.project_name}")
        if not generated_files:
            self.logger.warning("No generated files provided to Reviewer Agent. Skipping review.")
            return True, None # Pass if there's nothing to review

        if not impl_content:
             self.logger.error("Implementation plan content not provided. Cannot perform effective AI review.")
             # Decide how to handle: fail, or review without plan context? Let's fail.
             raise ValueError("Implementation plan content is required for AI review.")
        ...
        if not source_code_content:
             ...
             return False, "Review failed: Could not read source code files."

        # --- Perform AI Review ---
        self.logger.info("Attempting to perform code review using AI...")
        try:
            review_passed, feedback = self._perform_ai_review(impl_content, source_code_content)
            if review_passed:
                self.logger.info("AI Review passed.")
                return True, None
            else:
                self.logger.warning("AI Review found issues.")
                self.logger.debug(f"AI Feedback:\n{feedback}")
                return False, feedback
        except (ValueError, ConnectionError, RuntimeError) as e:
            self.logger.error(f"AI Review failed: {e}")
            # Treat review failure as build failure
            return False, f"AI Review failed: {e}"
        except Exception as e:
            self.logger.error(f"An unexpected error occurred during AI review: {e}", exc_info=True)
            return False, f"An unexpected error occurred during AI review: {e}"


    def _perform_ai_review(self, impl_content: str, source_code: dict[str, str]) -> tuple[bool, str | None]:
